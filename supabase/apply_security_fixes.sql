-- ====================================================
-- SECURITY FIXES — COPY-PASTE THIS FILE INTO THE SUPABASE SQL EDITOR
-- ====================================================
-- (Or run: `supabase db push` — the identical content is also committed as
-- supabase/migrations/20260810000000_fix_security_findings.sql, so this file
-- exists purely as a convenient source to copy from for the live project.)
--
-- Idempotent: policy drops use IF EXISTS, the realtime publication change is
-- guarded by a DO block. Safe to apply to the current live database.
--
-- Supersedes the ad-hoc policies created in `supabase/fix_all.sql`. Depends on
-- the base schema + `20260802000000_harden_rls_and_integrity.sql`. Backwards
-- compatible with the API: the backend writes through the service-role client,
-- which bypasses RLS, so the stricter policies do not change backend behavior.
--
